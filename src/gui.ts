...
<div class="gui-panel">
  ...
  <div class="gui-section">
    ...
    <div class="model-desc" id="shape-desc">Choose a primitive or one of the OBJ meshes from the assignment.</div>
  </div>
  ...
  overlay.querySelector(".gui-panel")!.insertAdjacentHTML(
    "beforeend",
    `
  <div class="gui-section">
    <div class="gui-label">Display</div>
    <label class="checkbox-row">
  ...
  document.querySelectorAll<HTMLButtonElement>(".shape-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      document.getElementById("shape-desc")!.textContent =
        shape === "cube"
          ? "Student test primitive cube."
          : shape === "sphere"
            ? "Student test primitive sphere."
            : shape === "beacon"
              ? "Loads beacon.obj and fits it with the assignment bounding sphere."
              : "Loads teapot.obj and fits it with the assignment bounding box.";
      onShapeChange(shape);
    });
  });

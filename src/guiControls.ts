...
export function updateZoomDisplay(value: number) {
  const slider = document.getElementById("zoom") as HTMLInputElement | null;
...
export function initGUI(options: {
  onShapeChange: (shape: GuiShape) => void;
  onZoomChange: (value: number) => void;
  onTextureFile: (file: File | null) => void;
  onUseDefaultTexture: () => void;
}) {
...
<div class="gui-panel">
  ...
  <div class="gui-section">
    ...
    ${slider("zoom", "Zoom", 0, 100, 1, 50)}
  </div>

  <div class="gui-section">
    <div class="gui-label">Texture</div>
    <input type="file" id="textureFile" accept="image/*">
    <button class="model-btn" id="defaultTextureBtn" type="button">Solid</button>
  </div>
  ...
  document.querySelectorAll<HTMLButtonElement>(".model-btn[data-id]").forEach(btn => {
    btn.addEventListener("click", () => {
      gui.modelId = Number(btn.dataset.id);
      document.querySelectorAll(".model-btn[data-id]").forEach(b => b.classList.remove("active"));
  ...
  (document.getElementById("textureFile") as HTMLInputElement)
    .addEventListener("change", e => {
      const file = (e.target as HTMLInputElement).files?.[0] ?? null;
      options.onTextureFile(file);
    });

  (document.getElementById("defaultTextureBtn") as HTMLButtonElement)
    .addEventListener("click", () => { options.onUseDefaultTexture(); });
}
